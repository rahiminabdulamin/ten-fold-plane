...
grep -Fq 'docker compose logs --tail=100 grist api' "$script_path"
grep -Fq 'http://localhost:8484/api/workspaces/1' "$script_path"
grep -Fq '178.128.104.112' "$script_path"
grep -Fq 'test: ["CMD", "curl", "-fsS", "http://localhost:8484/status"]' "$compose_path"
